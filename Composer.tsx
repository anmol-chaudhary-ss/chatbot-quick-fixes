...
class Composer extends React.Component<ComposerProps, { isRecording: boolean; height: number }> {
  private textInput: React.RefObject<HTMLTextAreaElement>
  constructor(props) {
    super(props)
    this.textInput = React.createRef()
    this.state = { isRecording: false, height: 22 }
  }
  ...
  handleKeyUp = e => {
    const scrollHeight = this?.textInput?.current?.scrollHeight || 0
    if (scrollHeight !== this.state.height) this.setState({ height: scrollHeight })
  }
  render() {
